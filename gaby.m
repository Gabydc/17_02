...
pIx = 1:G.cells.num; bhpIx = G.cells.num + 1; qSIx = G.cells.num + 3;
bhpIx2 = G.cells.num + 2; qSIx2 = G.cells.num + 4;
...
while t < totTime,
    ...
    while (resNorm > tol) && (nit < maxits)
        ...
        eqs{4} = qS_ad - sum(wellRates{1}(p_ad, bhp_ad))/rhoS;
        eqs{5} = qS_ad2 - sum(wellRates{2}(p_ad, bhp_ad2))/rhoS;
